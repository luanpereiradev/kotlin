...
abstract class FirDataFlowAnalyzer<FLOW : Flow>(
    ...
    private fun processEq(
        ...
        fun operationStatementTester(operationStatement: OperationStatement, block: () -> Unit) {
            // Only if operation statement is == True, i.e., left == right
            if ((operation.isEq() && operationStatement.operation == Operation.EqTrue) ||
        ...
        when {
            ...
            leftIsNullable -> processEqNull(node, leftOperand, operation.invert(), ::operationStatementTester)
            rightIsNullable -> processEqNull(node, rightOperand, operation.invert(), ::operationStatementTester)
        }
    ...
    private fun processEqNull(
        ...
        operationStatementTester: (OperationStatement, () -> Unit) -> Unit = { _, block -> block() }
    ) {
        val flow = node.flow
        val expressionVariable = variableStorage.createSyntheticVariable(node.fir)
        val operandVariable = variableStorage.getOrCreateVariable(node.previousFlow, operand)
        ...
        logicSystem.approveOperationStatement(flow, predicate).forEach { effect ->
            operationStatementTester.invoke(expressionVariable eq true) {
                flow.addImplication((expressionVariable eq true) implies effect)
            }
            operationStatementTester.invoke(expressionVariable eq false) {
